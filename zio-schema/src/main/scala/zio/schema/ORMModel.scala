...
class ORMModel extends scala.annotation.StaticAnnotation {

  def macroTransform(annottees: Any*): Any =
    macro ORMModelMacros.ormDocumentMacro
}

private[schema] class ORMModelMacros(val c: blackbox.Context) extends DerivationJsonTrait {

  import c.universe._

  def ormDocumentMacro(annottees: Tree*): Tree =
    constructORMDocumentCodec(annottees: _*)

  final def constructORMDocumentCodec(annottees: Tree*): Tree = annottees match {
...
//    println(showRaw(q"val _es:_root_.elasticsearch.orm.ElasticSearchDocument[Person]=Person"))
