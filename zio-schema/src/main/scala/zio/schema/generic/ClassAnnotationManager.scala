...
private class ClassAnnotationManager(
   ...
   * @return
   *   a columnar Json.Obj with columnar data
   */
  def extractColumnarDescription(
    ...
    annotations.foreach {
      case nc: NoColumnar      => fields ::= ACTIVE -> Json.Bool(false)
      case ColumnFamily(value) => fields ::= FAMILY -> Json.Str(value)
      case ColumnQualifier(value) =>
        fields ::= QUALIFIER -> Json.Str(value)
      case TableName(value) => fields ::= TABLE -> Json.Str(value)
      case NamespaceName(value) =>
        fields ::= NAMESPACE -> Json.Str(value)
//      case c:ColumnVisibilityAnnotation => visibilities ::= c.asJson
      ...
      case _: ColumnarSingleJson =>
        fields ::= IS_SINGLE_JSON -> Json.Bool(true)
      case SingleStorage(singleStorageName) =>
        fields ::= SINGLE_STORAGE -> Json.Str(singleStorageName)
    }
