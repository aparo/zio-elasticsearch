...
trait JsonDiffSupport[JsValue] { this: DiffsonInstance[JsValue] =>
  ...
  class JsonDiff(lcsalg: Lcs[JsValue]) {
    ...
    private def diff(
      ...
      (json1, json2) match {
        case (v1, v2) if v1 == v2 =>
          Nil // if they are equal, this one is easy...
        ...
        case (JsArray(arr1), JsArray(arr2)) if arrayDiffs =>
          arraysDiff(arr1.toList, arr2.toList, remember, pointer)
        case (_, _) =>
          List(Replace(pointer, json2, if (remember) Some(json1) else None))
      }
    ...
    private def fieldsDiff(
      ...
      def fields(
        ...
        case (Some(f1), Some(f2)) :: tl =>
          ...
          fields(
            tl,
            diff(f1._2, f2._2, remember, arraysDiff, path / f1._1) ::: acc
          )
        case (Some(f1), None) :: tl =>
          // the field was deleted
          fields(
            tl,
            Remove(path / f1._1, if (remember) Some(f1._2) else None) :: acc
          )
