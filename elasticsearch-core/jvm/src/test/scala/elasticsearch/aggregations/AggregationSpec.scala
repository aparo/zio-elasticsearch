...
//    val precents = Array(95, 99, 99.9)
  ...
  it should "deserialize percentileRanks_missingValue" in {
    ...
    myagg.field should be("grade")
    myagg.missing.isDefined should be(true)
    myagg.missing.get should be(Json.fromInt(10))
  }
...
//    miss hdr attribute
  ...
  it should "deserialize range" in {
    ...
    myagg.field should be("price")
    myagg.ranges.head.to should be(Some(Json.fromInt(50)))
    myagg.ranges(1).from should be(Some(Json.fromInt(50)))
    myagg.ranges(1).to should be(Some(Json.fromInt(100)))
    myagg.ranges(2).from should be(Some(Json.fromInt(100)))
  }
  ...
  it should "deserialize range_valueScript" in {
    ...
    val myagg = aggregations.head._2.asInstanceOf[RangeAggregation]
    myagg.field should be("price")
    myagg.ranges.head.to should be(Some(Json.fromInt(35)))
    myagg.ranges(1).from should be(Some(Json.fromInt(35)))
    myagg.ranges(1).to should be(Some(Json.fromInt(70)))
    myagg.ranges(2).from should be(Some(Json.fromInt(70)))
    myagg.script.isDefined should be(true)
    val myscript = myagg.script.get.asInstanceOf[InlineScript]
    myscript.source should be("_value * params.conversion_rate")
    myscript.lang should be("painless")
  ...
  it should "deserialize range_subAggregations" in {
    ...
    myagg1.ranges.head.to should be(Some(Json.fromInt(50)))
    myagg1.ranges(1).from should be(Some(Json.fromInt(50)))
    myagg1.ranges(1).to should be(Some(Json.fromInt(100)))
    myagg1.ranges(2).from should be(Some(Json.fromInt(100)))
  ...
  it should "deserialize dateRange" in {
    ...
    myagg.ranges.head.to should be(Some(Json.fromString("now-10M/M")))
    myagg.ranges(1).from should be(Some(Json.fromString("now-10M/M")))
  }
  ...
  it should "deserialize filter" in {
    ...
    myFilter.value should be(Json.fromString("t-shirt"))
...
//    myagg.origin.toString should be("52.3760, 4.894")
    myagg.ranges.head.to should be(Some(Json.fromInt(100000)))
    myagg.ranges(1).from should be(Some(Json.fromInt(100000)))
    myagg.ranges(1).to should be(Some(Json.fromInt(300000)))
    myagg.ranges(2).from should be(Some(Json.fromInt(300000)))
  }
  ...
  it should "deserialize ipRange" in {
    ...
    myagg.ranges.head.to should be(Some(Json.fromString("10.0.0.5")))
    myagg.ranges(1).from should be(Some(Json.fromString("10.0.0.5")))
  }
...
//    myagg.field should be("remote_ip")
//    myagg.ranges.head.to should be(Json.fromString("10.0.0.5"))
//    myagg.ranges(1).from should be(Json.fromString("10.0.0.5"))
//    myagg.keyed should be(true)
//  }
