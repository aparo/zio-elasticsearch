...
final case class TermVectorsResult(
  ...
  @jsonField("term_vectors") termVectors: Option[Map[String, TermVector]] = None,
  error: Option[ErrorCause] = None
)

object TermVectorsResult {
  implicit lazy val jsonCodec: JsonCodec[TermVectorsResult] =
    DeriveJsonCodec.gen[TermVectorsResult]
}
