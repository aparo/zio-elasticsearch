...
trait ElasticSearchService
  ...
  )(implicit encoder: Encoder.AsObject[T]): ZSink[Any, Throwable, T, T, Long] = {
  ...
  def buildIndexRequest[T](
    ...
    IndexRequest(
      index = realIndex,
      body =
        if (extra.nonEmpty) JsonObject.fromIterable(item.asJsonObject.toList ++ extra.toList) else item.asJsonObject,
      id = id,
      opType =
