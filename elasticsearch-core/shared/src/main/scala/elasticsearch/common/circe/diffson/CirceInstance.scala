...
class CirceInstance extends DiffsonInstance[Json] {
  ...
  trait DiffsonProtocol {
    ...
    implicit val pointerDecoder: Decoder[JsonPointer] =
      Decoder[String].emap { s =>
        Either.catchNonFatal(JsonPointer.parse(s)).leftMap(_.getMessage)
      }
    ...
    implicit val operationDecoder: Decoder[Operation] =
      new Decoder[Operation] {
        ...
        override def apply(c: HCursor): Result[Operation] =
          F.flatMap(
            c.get[String]("op").leftMap(_.withMessage("missing 'op' field"))
          ) {
            case "add" =>
              A.map2(c.get[JsonPointer]("path"), c.get[Json]("value"))(Add)
                .leftMap(_.withMessage("missing 'path' or 'value' field"))
            case "remove" =>
              A.map2(c.get[JsonPointer]("path"), c.get[Option[Json]]("old"))(
                  Remove
                )
                .leftMap(_.withMessage("missing 'path' or 'old' field"))
            case "replace" =>
              A.map3(
                  c.get[JsonPointer]("path"),
                  c.get[Json]("value"),
                  c.get[Option[Json]]("old")
                )(Replace)
                .leftMap(_.withMessage("missing 'path' or 'value' field"))
            case "move" =>
              A.map2(c.get[JsonPointer]("from"), c.get[JsonPointer]("path"))(
                  Move
                )
            ...
            case other =>
              Left(
                DecodingFailure(s"""Unknown operation "$other"""", c.history)
              )
          }
      }
  ...
  object provider extends JsonProvider {
    ...
    def parseJson(s: String): Json =
      io.circe.parser.parse(s).valueOr(throw _)
