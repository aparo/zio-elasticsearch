...
case class TypedQueryBuilder[T](
  ...
  def getLastUpdate(field: String): EitherT[Future, FrameworkException, Option[OffsetDateTime]] = {
    ...
    implicit val executor: ExecutionContext = nosqlContext.elasticsearch.executionContext
  ...
  def highlight(highlight: Highlight): TypedQueryBuilder[T] = this.copy(highlight = highlight)
  ...
  def indices(indices: Seq[String]): TypedQueryBuilder[T] = this.copy(indices = indices)

  def index(index: String): TypedQueryBuilder[T] = this.copy(indices = Seq(index))

  def types(types: Seq[String]): TypedQueryBuilder[T] = this.copy(docTypes = types)

  def `type`(`type`: String): TypedQueryBuilder[T] = this.copy(docTypes = Seq(`type`))

  def setFields(fields: Seq[String]): TypedQueryBuilder[T] = this.copy(fields = fields)
  ...
  def noSource: TypedQueryBuilder[T] = this.copy(source = SourceSelector.noSource)
  ...
  def sortBy(sort: Sorter): TypedQueryBuilder[T] = this.copy(sort = this.sort ::: sort :: Nil)

  def sortBy(field: String, reverse: Boolean = false): TypedQueryBuilder[T] =
    this.copy(sort = this.sort ::: FieldSort(field, SortOrder(false)) :: Nil)

  def withFilter(projection: T => Boolean): TypedQueryBuilder[T] =
    macro QueryMacro.filter[T]

  def toList: EitherT[Future, FrameworkException, List[T]] =
    results.map(_.hits.toList.map(_.source))

  def getOrElse(default: T): EitherT[Future, FrameworkException, T] =
    this.get.map {
      case Some(d) => d
      case None => default
    }

  def getOrCreate(default: T): EitherT[Future, FrameworkException, (Boolean, T)] =
    this.get.map {
      case Some(d) => (false, d)
      case None =>
        (true, default)
  ...
  def get: EitherT[Future, FrameworkException, Option[T]] =
    this.toVector.map { result =>
      result.length match {
        case 0 => None
        case 1 => Some(result.head)
        case _ => throw new MultiDocumentException("Multi value returned in get()")
      }
    }

  def toVector: EitherT[Future, FrameworkException, Vector[T]] =
    results.map(_.hits.toVector.map(_.source))

  def results: EitherT[Future, FrameworkException, SearchResult[T]] =
  ...
  def delete(): Unit = {
    ...
    client.awaitResult(refresh.value)
    ()
  }

  def refresh(implicit nosqlContext: ESNoSqlContext): EitherT[Future, FrameworkException, RefreshResponse] = {
    implicit val client = nosqlContext.elasticsearch
  ...
  def setScan(scrollTime: String = "5m"): TypedQueryBuilder[T] =
    this.copy(
      sort = this.sort ::: FieldSort("_doc", SortOrder.Asc) :: Nil, // TODO only a single _doc in sort
      searchType = Some("scan"),
      scrollTime = Some(scrollTime),
      suggestions = Map(),
      aggregations = Map()
  ...
  def multiGet(ids: List[String]): EitherT[Future, FrameworkException, List[ResultDocument[T]]] =
    client
      .mget[T](index = this.getRealIndices(indices).head, docType = this.docTypes.head, ids = ids)

  def update(doc: JsonObject): Int = update(doc, true, true)

  def update(doc: JsonObject, bulk: Boolean, refresh: Boolean): Int = {
    var count = 0
    ...
    for (r <- scan) {
      ...
      if (bulk)
        ...
        client.awaitResult(client.update(ur).value)
      count += 1
    }
  ...
  def update(func: T => Option[T], refresh: Boolean = false): Int = {
    ...
    for (r <- scan) {
      ...
      if (newRecord.isDefined) {
        client.addToBulk(IndexRequest(r.index, r.docType, Some(r.id), body = (newRecord.get).asJson.asObject.get))
        count += 1
      }
    }
  ...
  def fromList(list: List[T]): TypedQueryBuilder[T] = new ListTypedQueryBuilder[T](list)
  ...
  def reverseSortBy(projection: T => Any): TypedQueryBuilder[T] = macro QueryMacro.reverseSortBy[T]

  def withQuery(projection: T => Boolean): TypedQueryBuilder[T] = macro QueryMacro.filter[T]

  //STREAM API
  //
  //  def toSourceResultDocument(
  //      scrollKeepAlive: FiniteDuration = 600.seconds
  //  ): Source[ResultDocument[T], akka.NotUsed] = {
  //    implicit val actorSystem = nosqlContext.akkaSystem
  //    var query = this
  //    if (!query.isScroll) {
  //      query = query.setScan()
  //    }
  //
  //    Source.fromPublisher(nosqlContext.elasticsearch.publisher[T](query))
  //
  //  }
  //
  //  def toSource(
  //      scrollKeepAlive: FiniteDuration = 600.seconds
  //  ): Source[T, akka.NotUsed] =
...
object TypedQueryBuilder {
  def fromSchema[T: Encoder: Decoder](implicit nosqlContext: ESNoSqlContext,
                                      jsonSchema: JsonSchema[T]): TypedQueryBuilder[T] = {
    val schema = jsonSchema.asSchema
    new TypedQueryBuilder[T](indices = Seq(schema.tableName), schema = Some(schema))
  }
}
...
class ListTypedQueryBuilder[T: Encoder: Decoder](val items: List[T])(implicit override val nosqlContext: ESNoSqlContext)
  ...
  override def count: EitherT[Future, FrameworkException, Long] =
    EitherT.right(Future.successful(items.length.toLong))

  override def length: EitherT[Future, FrameworkException, Long] =
    EitherT.right(Future.successful(items.length.toLong))
...
class EmptyTypedQueryBuilder[T: Encoder: Decoder]()(implicit override val nosqlContext: ESNoSqlContext)
    extends TypedQueryBuilder[T]() {
  override def count: EitherT[Future, FrameworkException, Long] = EitherT.right(Future.successful(0L))

  override def length: EitherT[Future, FrameworkException, Long] = EitherT.right(Future.successful(0L))
}
