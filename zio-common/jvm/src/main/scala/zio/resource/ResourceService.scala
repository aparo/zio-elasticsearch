...
object ResourceService {
  ...
  trait Service {
    def getFileAsString(name: String,
                        codec: Codec = Codec.UTF8): ZIO[Any, Throwable, String]
    def getJson(name: String,
                codec: Codec = Codec.UTF8): ZIO[Any, Throwable, Json]
    def getJsonEntity[T: Decoder](
        name: String,
        codec: Codec = Codec.UTF8): ZIO[Any, Throwable, T]
  }

  def getFileAsString(
      name: String,
      codec: Codec = Codec.UTF8): ZIO[ResourceService, Throwable, String] =
    ZIO.accessM(_.get.getFileAsString(name, codec))

  def getJson(
      name: String,
      codec: Codec = Codec.UTF8): ZIO[ResourceService, Throwable, Json] =
  ...
  object Live extends ResourceService.Service {
    override def getFileAsString(name: String,
                                 codec: Codec = Codec.UTF8): Task[String] = {
