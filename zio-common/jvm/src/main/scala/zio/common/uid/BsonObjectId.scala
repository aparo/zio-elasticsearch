...
object BsonObjectId {
  ...
  private val machineId = {
    ...
    val permitted = {
      ...
      Try { sec.checkPermission(new NetPermission("getNetworkInformation")) }.toOption
        .map(_ => true)
        .getOrElse(false);
    }
  ...
  def fromTime(
      timeMillis: Long,
      fillOnlyTimestamp: Boolean = true
  ): BsonObjectId = {
