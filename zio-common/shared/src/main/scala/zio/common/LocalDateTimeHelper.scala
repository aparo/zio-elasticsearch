...
object LocalDateTimeHelper extends DateTimeHelper {
   ...
   * @return
   *   LocalDateTime instance of the current instant
   */
  ...
  /**
   * Create a LocalDateTime at midnight of the date "yyyyMMdd" specified in the
   * string date passed.
   * @param strDate
  ...
  /**
   * Parse the date in string format to LocalDateTime using one of these pattern
   * parsersISODateTime
   *
   * @param strDate
   *   the date
   * @return
   *   an Either[Throwable, LocalDateTime]
   */
  def parseToLDT(strDate: String): Either[Throwable, LocalDateTime] = {
    var result: Option[Either[Throwable, LocalDateTime]] = None
    parsers.exists { dt =>
      try {
    ...
    result.getOrElse(Left(ParsingFailureException(s"Date in $strDate", s"Date in $strDate")))
  }
