...
import zio._
import zio.elasticsearch.security.UserProfileWithMetadata
import zio.json._
...
final case class GetUserProfileResponse(
  profiles: Chunk[UserProfileWithMetadata] = Chunk.empty[UserProfileWithMetadata],
  errors: GetUserProfileErrors
) {}
object GetUserProfileResponse {
  implicit lazy val jsonCodec: JsonCodec[GetUserProfileResponse] =
    DeriveJsonCodec.gen[GetUserProfileResponse]
}
