...
class MappingMerger() {
  ...
  /**
   * Try to merge two mapping. Return new mapping or exception and way to fix in
   * other case
   * @param sourceName
   *   the current mapping name
   * @param sourceMapping
   *   the source mapping
   * @param destName
   *   the other mapping name
   * @param destMapping
   *   the mapping to merge
   * @return
   *   The merged mapping or the an an Exception
   */
  def merge(
    sourceName: String,
  ...
  /**
   * Try to merge two mappings of Nested/Object/RootDocument. Return new
   * mapping/properties or exception and way to fix in other case
   * @param sourceName
   *   the current mapping name
   * @param sourceMapping
   *   the source mapping
   * @param sourceProperties
   *   the source properties of the mapping
   * @param destName
   *   the other mapping name
   * @param destMapping
   *   the mapping to merge
   * @param destProperties
   *   the properties of the other mapping
   * @return
   *   The merged mapping or the an an Exception
   */
    ...
    if (patch.ops.isEmpty) {
      ...
      patch.ops.foreach {
        case op @ Replace(path, value, old) =>
          val realPath = path.toString.substring(1)
          if (realPath == "type") {
