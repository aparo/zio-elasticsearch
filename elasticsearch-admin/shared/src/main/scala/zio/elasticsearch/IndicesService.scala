...
import zio.elasticsearch.client.IndicesActionResolver
...
trait IndicesService extends IndicesActionResolver {
  ...
  protected var defaultSettings = Settings.ElasticSearchBase
  protected var defaultTestSettings = Settings.ElasticSearchTestBase
  ...
  def flushBulk(): ZioResponse[IndicesFlushResponse] =
    for {
      blkr <- client.bulker
      _ <- blkr.flushBulk()
