...
allocate or fail shard) which have not yet been executed.
    ...
    lazy val mappings =
      new elasticsearch.mappings.MappingManager()(logger, indicesService, this)
    ...
    def get(
      ...
      logger.log(LogLevel.Debug)(s"get($ri, $id)") *> (authContext.user match {
    ...
    def delete(
      ...
      for {
        _ <- logger.log(LogLevel.Debug)(s"delete($ri, $id)")
  ...
  val live: ZLayer[IndicesService, Nothing, Has[Service]] =
    ZLayer.fromService[IndicesService.Service, Service] { (indicesService) =>
      Live(indicesService, indicesService.logger, indicesService.httpService, indicesService.client)
    }
