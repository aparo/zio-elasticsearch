...
abstract class TestJsonPatch[JsValue, Instance <: DiffsonInstance[JsValue]](
  ...
  it should "create a nested field if needed" in {
    ...
    op(parseJson("{ \"lbl\": {} }")) should be(
      parseJson("{ \"lbl\": { \"lbl\": 17 } }")
    )
  }
  ...
  "removing a label of an object" should "result in the object being amputed from this label" in {
    ...
    op(parseJson("{ \"lbl\": 17, \"toto\": true }")) should be(
      parseJson("{ \"toto\": true }")
    )
  }
  ...
  "replacing an element in an object" should "result in this element being replaced" in {
    ...
    op(parseJson("""{"lbl": {"lbl": true, "gruik": 1}, "toto": 3}""")) should be(
  ...
  "replacing an element in an array" should "result in this element being replaced" in {
    ...
    op(parseJson("[true, false, true, true, true]")) should be(
      parseJson("[true, false, true, 17,true]")
    )
  }
