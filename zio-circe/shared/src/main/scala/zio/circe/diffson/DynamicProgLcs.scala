...
class DynamicProgLcs[T] extends Lcs[T] {
  ...
  def lcs(
    ...
    if (seq1.isEmpty || seq2.isEmpty) {
      ...
      val (prefix, middle1, middle2, suffix) =
        splitPrefixSuffix(seq1, seq2, low1, low2)
      val indexedMiddle1: Vector[T] = middle1.toVector
      val indexedMiddle2: Vector[T] = middle2.toVector
      val offset = prefix.size
      val lengths: Array[Array[Int]] =
        Array.ofDim[Int](middle1.size + 1, middle2.size + 1)
      ...
      def loop(idx1: Int, idx2: Int, acc: List[(Int, Int)]): List[(Int, Int)] =
        if (idx1 == 0 || idx2 == 0) {
          ...
          loop(
            idx1 - 1,
            idx2 - 1,
            (low1 + offset + idx1 - 1, low2 + offset + idx2 - 1) :: acc
          )
        }
  ...
  private def splitPrefixSuffix(
    ...
    (
      prefix,
      seq1.drop(prefix.size).dropRight(suffix.size),
      seq2.drop(prefix.size).dropRight(suffix.size),
      suffix
    )
  }
